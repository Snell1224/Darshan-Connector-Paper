...
We created an I/O targeted LDMS streams store that utilizes the LDMS publish-subscribe bus capability, \emph{LDMS Streams}, to publish I/O event data. \emph{LDMS Streams} is intended for publishing and subscribing to an \emph{LDMS streams tag}. 
...
DSOS is built on the Scalable Object Store (SOS) database~\cite{sosgithub} and was intended to address the domain-specific needs of large-scale HPC monitoring. It was chosen as the preferred monitoring database at Sandia because it allows for interaction via a command line interface which allows for fast query testing and data examination. DSOS also provides scalable data ingest and the ability to query large volumes of data which is required for the large amounts of data to be ingested and stored. %However, a different storage that had similar capabilities could be used instead. 
